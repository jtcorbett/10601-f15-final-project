function model = build_PNN(data, labels, parameters)
  ...
  for epoch_i=1:epoch_num
    ...
    for batch_start=1:batch_size:size(data, 1)-batch_size
      batch_id = ceil(batch_start/batch_size);
      ...
      for layer_i=2:L
        batch_weights{layer_i} = zeros(layers(layer_i), layers(layer_i-1), batch_size);
        batch_biases{layer_i} = zeros(layers(layer_i), batch_size);
      end  
      ...
      for sample_i=1:batch_size
        ...
        for layer_i=2:L
          weight_cell = batch_weights{layer_i};
          weight_cell(: , :, batch_id) = weight_cell(: , :, batch_id) + (deltas{layer_i}*activation{layer_i-1}'); %'
          batch_cell = batch_biases{layer_i};
          batch_cell(:, batch_id) = batch_cell(:, batch_id) + deltas{layer_i};
        end
      end
      ...
      old_weights = weights;
      for layer_i=2:L
        weights{layer_i} = weights{layer_i} - (learning_rate/batch_size)*sum(batch_weights{layer_i}, 3);
        biases{layer_i} = biases{layer_i} - (learning_rate/batch_size)*sum(batch_biases{layer_i}, 2);
      end
    ...
    for sample_i=1:eval_size
      ...
      e(sample_i) = hinge_error(classes, output, ans);
      if guess == ans;
        good(sample_i) = 1;
        % "right"
        % sample_j
